...
- Nav2 负责重新规划和受控倒车；Ackermann 底盘不能原地自转，控制台提供经实车验证的受控弧线转向
- 通过地图点击或输入 `x / y / yaw` 发送地图坐标目标
- 紧急停止、暂停、继续、取消目标、倒车、Ackermann 弧线转向、保存地图均已做实车闭环验证；STOP 会取消全部 Nav2 目标并关闭 AI 自动下发
- 电脑端每秒采样一帧，只保留最新连续帧；本机 Qwen3-VL 实测一轮结构化决策约 15 秒，旧提示词/旧代次的迟到结果不会下发
...
- 实时提示词会立即隔离旧推理；以“立即停止/暂停/继续”开头时直接执行对应控制，不必等待模型推理
...
脚本会跳过已运行的同一套系统，避免重复启动 ROS 节点；发现厂商 YOLO 演示栈时会先清理冲突。需要重新加载已部署代码时使用：

```powershell
.\deploy_robot_autonomy.ps1 -Start -Restart
```
